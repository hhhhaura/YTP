...
class Puzzle():
    ...
    def __init__(self, path, viewer=None, green_screen=False):
        ...
        start_piece = self.connected_pieces[0]
        ...
        self.connected_pieces = self.solve(self.connected_pieces, border_pieces)
        self.log('>>> START solve middle')
        self.strategy = Strategy.FILL
        self.solve(self.connected_pieces, non_border_pieces)
    ...
    def solve(self, connected_pieces, left_pieces, border=False):
        ...
        if len(self.connected_directions) == 0:
            ...
            self.diff = self.compute_diffs(self.left_pieces, self.diff, connected_pieces[0]) # edge on the border of the block -> edge on a left piece -> diff between edges
        else:
            self.diff = self.add_to_diffs(self.left_pieces)

        while len(self.left_pieces) > 0:
            self.log("<--- New match ---> pieces left: ", len(self.left_pieces), 'extremum:', self.extremum, 'puzzle dimension:', display_dim(self.possible_dim))
            self.set_vis(self.diff) #set the visited label to False
            self.best_diff(self.diff, self.connected_directions, self.left_pieces)
        return self.connected_pieces
    ...
    def compute_diffs(self, left_pieces, diff, new_connected, edge_connected=None):
        ...
        for e in new_connected.edges_:
            ...
            diff[e] = diff_e
        return diff
    def check(self, block_best_e, best_e):
        #backup
        '''
        1. block_best_p, block_best_e, best_e, best_p
        2. connected_pieces, left_pieces, connected_directions, self.extremum, self.possible_dims
        3. self.corner_pos
        '''
        block_best_p, best_p = self.edge_to_piece[block_best_e], self.edge_to_piece[best_e]
        tp_block_best_p = copy.deepcopy(block_best_p)
        tp_block_best_e = copy.deepcopy(block_best_e)
        tp_best_e = copy.deepcopy(best_e)
        tp_best_p = copy.deepcopy(best_p)
        ...
        yes = int(input('Is the piece correct?\n'))
        if yes == 1 : 
            ...
            self.connected_pieces.append(best_p)
            del self.left_pieces[self.left_pieces.index(best_p)]
            self.diff = self.compute_diffs(self.left_pieces, self.diff, best_p, edge_connected=block_best_e)
        else :
            block_best_p.change(tp_block_best_p)
            block_best_e.change(tp_block_best_e)
            best_e.change(tp_best_e)
            best_p.change(tp_best_p)
            #print(best_p == tp_best_p)
            self.export_pieces('/tmp/stick{0:03d}'.format(len(self.connected_directions)) + ".png",
                             '/tmp/colored{0:03d}'.format(len(self.connected_directions)) + ".png",
                                name_colored='Step {0:03d}'.format(len(self.connected_directions)))
        return yes
    ...
    def fallback(self, diff, connected_direction, left_piece, strat=Strategy.NAIVE):
        ...
        yes = self.best_diff(diff, connected_direction, left_piece)
    ...
    def best_diff(self, diff, connected_direction, left_piece):
        ...
        if self.strategy == Strategy.FILL:
            ...
            if not best_e is None and self.check(best_bloc_e, best_e) == 1:
                return 1
            if not best_e is None :
                self.vis[best_bloc_e][best_e] = 1
            while self.fallback(diff, connected_direction, left_piece) == 0: 
                continue
            return 1
                ...
                for p in left_piece:
                    for rotation in range(4):
                        ...
                        if diff_score != 0 or edge_exposed.connected or edge.connected \
                            ...
                            #print(edge.type, edge_exposed.type, (edge_exposed in diff))
                            diff_score = diff[edge_exposed][edge]
            ...
            for block_e, block_e_diff in diff.items():
                for e, diff_score in block_e_diff.items():
                    if diff_score < min_diff and self.vis[block_e][e] == 0:
                        best_bloc_e, best_e, min_diff = block_e, e, diff_score
            self.vis[best_bloc_e][best_e] = 1
            #print(min_diff)
            yes = self.check(best_bloc_e, best_e)
            return yes
        else:
            return 0
