...
class _PieceProcessor:
    ...
    def processEdge(self, id):
        ...
        contourMask = np.zeros_like(self.originImage)
        temp = shiftContour(self.contour, minX, minY)
        cv2.drawContours(contourMask, [temp], 0, (255, 255, 255), -1)
        for rp in interpolation : 
            rp[0] = rp[0] * p2relativep1[0]
            rp[1] = rp[1] * p2relativep1[0]
            vec = cartesianToPolar(rp)
            vec = (vec[0], vec[1] + p2relativep1[1])
            vec = polarToCartesian(vec)
            p = p1 + vec
            p[0] = round(p[0])
            p[1] = round(p[1])
            allColor = []
            for dx in range(-Config.circleLen, Config.circleLen + 1) :
                for dy in range(-Config.circleLen, Config.circleLen + 1) :
                    tx = int(p[0] + dx)
                    ty = int(p[1] + dy)
                    yes = True
                    yes = yes and tx >= 0 and ty >= 0
                    yes = yes and (ty < self.originImage.shape[0])
                    yes = yes and (tx < self.originImage.shape[1])
                    yes = yes and (contourMask[ty - minY][tx - minX][0] > 0)
                    if not yes : 
                        continue
                    allColor.append(self.originImage[ty][tx])
            colors.append(np.median(allColor, axis=0))
        edge.color = colors    
        return edge
